...
int DecryptPassword(gxString &password, unsigned char *pbuf, 
		    unsigned plen)
{
  /*
  password.Clear();
  AES_buf_t b;
  unsigned  offset = 0;
  b.salt.resize(SALT_LENGTH(b.mode));
  memmove(b.salt.m_buf(), pbuf, SALT_LENGTH(b.mode)); 
  offset += SALT_LENGTH(b.mode);
  b.key.resize(AES_KEY_LEN);
  memmove(b.key.m_buf(), (pbuf+offset), AES_KEY_LEN); 
  offset += AES_KEY_LEN;
  b.verifier.resize(PWD_VER_LENGTH);
  memmove(b.verifier.m_buf(), (pbuf+offset), PWD_VER_LENGTH);
  offset += PWD_VER_LENGTH;
  b.MAC.resize(MAC_LENGTH(b.mode));
  memmove(b.MAC.m_buf(), (pbuf+offset), MAC_LENGTH(b.mode));
  offset += MAC_LENGTH(b.mode);
  b.buf.resize(plen-offset);
  memmove(b.buf.m_buf(), (pbuf+offset), (plen-offset));

  int rv = AES_decrypt_buf(&b);
  if(rv != 0) {
    b.Clear();
    return rv;
  }
  password.SetString((char *)b.buf.m_buf(), b.buf.length());
  */
  return AES_NO_ERROR;
}

int CryptPassword(gxString &password, unsigned char *pbuf,
		  unsigned &plen)
{
  /*

  AES_buf_t b;
  if(!pbuf) pbuf = new unsigned char[1024];
  if(!pbuf) return AES_ERROR_OUT_OF_MEMORY;
  plen = 0;

  strcpy(b.secret, password.GetSPtr());
  b.secret_len = password.length()
  memmove(pbuf, password.GetSPtr(), password.length());
  len = password.length();

  int rv = AES_crypt_buf(&b, (void *)pbuf, len);
  if(rv != 0) {
    password.Clear(1);
    return rv;
  }
  ...
  password.Clear(1);
  memmove(pbuf, b.salt.m_buf(), SALT_LENGTH(b.mode));
  plen += SALT_LENGTH(b.mode);
  memmove((pbuf+plen), b.key.m_buf(), AES_KEY_LEN);
  plen += AES_KEY_LEN;
  memmove((pbuf+plen), b.verifier.m_buf(), PWD_VER_LENGTH);
  plen += PWD_VER_LENGTH;
  memmove((pbuf+plen), b.MAC.m_buf(), MAC_LENGTH(b.mode));
  plen += MAC_LENGTH(b.mode);
  memmove((pbuf+plen), b.buf.m_buf(), b.buf.length());
  plen += b.buf.length();
  */
  return AES_NO_ERROR;
}
